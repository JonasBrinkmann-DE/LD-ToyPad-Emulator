...
0. Start the virtual machine if it's not already running. Then start the **VirtualHere USB Client** and double click on `LEGO READER V2.10`.

1. Make sure that you are in the LD-ToyPad-Emulator directory, if not run:

   ```bash
   cd LD-ToyPad-Emulator
   ```

2. Build the app, this has only to be done **once** or if any changes where made to any **.ts** file
   ```bash
   node build
   ```
3. Run the emulator server with this command if you are in the correct folder (otherwise run `cd    LD-ToyPad-Emulator` first):

   ```bash
   node start
   ```

4. Type `http://debian` in a browser to use the emulator.
...
5. Finally, start your console emulator and the game itself (e.g. Cemu).
...
4. Type **your single board computer's IP address** in a browser to use the emulator.
